...
def get_glory_from_wins(total_wins: int) -> int:
    ...
    return math.floor((10 * (45 * (math.log10(total_wins * 2) ** 2))) + 245)
...
def get_glory_from_best_rating(best_rating: int) -> int:
    ...
    if best_rating < 1200:
        retval = 250
    elif best_rating >= 1200 and best_rating < 1286:
        retval = 10 * (25 + (0.872093023 * (86 - (1286 - best_rating))))
    elif best_rating >= 1286 and best_rating < 1390:
        retval = 10 * (100 + (0.721153846 * (104 - (1390 - best_rating))))
    elif best_rating >= 1390 and best_rating < 1680:
        retval = 10 * (187 + (0.389655172 * (290 - (1680 - best_rating))))
    elif best_rating >= 1680 and best_rating < 2000:
        retval = 10 * (300 + (0.428125 * (320 - (2000 - best_rating))))
    elif best_rating >= 2000 and best_rating < 2300:
...
def get_hero_elo_from_old_elo(old_elo: int) -> int:
    ...
    """
    if old_elo < 2000:
        return math.floor((old_elo + 375) / 1.5)
    return math.floor(1583 + (old_elo - 2000) / 10)
...
def get_personal_elo_from_old_elo(old_elo: int) -> int:
    ...
    """
    if old_elo >= 1400:
        return math.floor(1400 + (old_elo - 1400.0) / (3.0 - (3000 - old_elo) / 800.0))
    return old_elo
